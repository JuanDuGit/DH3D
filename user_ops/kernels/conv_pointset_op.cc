...
class ConvPointsetOp : public OpKernel {
 public:
  explicit ConvPointsetOp(OpKernelConstruction* ctx) : OpKernel(ctx) {}
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    const int Dout = theta_.shape().dim_size(1);

    Tensor* output_ = nullptr;
    OP_REQUIRES_OK(
        ctx, ctx->allocate_output(0, TensorShape({B, Dout, N}), &output_));

    ::tensorflow::functor::ConvPointsetFunctor<Device, Dtype>()(
        ctx, features_, theta_, bias_, neighborhood_, output_);
  }

 private:
  TF_DISALLOW_COPY_AND_ASSIGN(ConvPointsetOp);
};

// Backward-Pass (CPU, GPU)
// --------------------------------------------------
template <typename Device, typename Dtype>
class ConvPointsetGradOp : public OpKernel {
 public:
  explicit ConvPointsetGradOp(OpKernelConstruction* ctx) : OpKernel(ctx) {}
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    const Tensor& topdiff_ = ctx->input(4);
    ...
    ::tensorflow::functor::ConvPointsetGrad<Device, Dtype>()(
        ctx, features_, theta_, bias_, neighborhood_, topdiff_,
...
REGISTER_CUSTOM_OP(ConvPointset, CPU, float);
REGISTER_CUSTOM_OP(ConvPointsetGrad, CPU, float);
REGISTER_CUSTOM_OP(ConvPointset, CPU, double);
REGISTER_CUSTOM_OP(ConvPointsetGrad, CPU, double);

#ifdef GOOGLE_CUDA
REGISTER_CUSTOM_OP(ConvPointset, GPU, float);
REGISTER_CUSTOM_OP(ConvPointsetGrad, GPU, float);
REGISTER_CUSTOM_OP(ConvPointset, GPU, double);
REGISTER_CUSTOM_OP(ConvPointsetGrad, GPU, double);
#endif  // GOOGLE_CUDA
